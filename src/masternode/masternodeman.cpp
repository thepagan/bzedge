...
CMasternode* CMasternodeMan::GetNextMasternodeInQueueForPayment(int nBlockHeight, bool fFilterSigTime, int& nCount)
{
    ...
    */
    ...
    for (CMasternode& mn : vMasternodes) {
        ...
        if (fFilterSigTime && mn.sigTime + (nMnCount * 2.6 * 60) > GetTime()) continue;
...
std::vector<pair<int, CMasternode> > CMasternodeMan::GetMasternodeRanks(int64_t nBlockHeight, int minProtocol)
{
    ...
    int rank = 0;
    for (const std::pair<int64_t, CMasternode>& s : vecMasternodeScores) {
        rank++;
...
void CMasternodeMan::ProcessMasternodeConnections()
{
    ...
    if (ChainNameFromCommandLine() == CBaseChainParams::REGTEST) return;
...
void CMasternodeMan::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "mnb") { //Masternode Broadcast
        ...
        if (vin == CTxIn()) { //only should ask for this once
            ...
            if (!isLocal && ChainNameFromCommandLine() == CBaseChainParams::MAIN) {
    ...
    // Light version for OLD MASSTERNODES - fake pings, no self-activation
        ...
        if (pmn != NULL) {
            ...
            if (count == -1 && pmn->pubKeyCollateralAddress == pubkey && (GetTime() - pmn->nLastDsee > MASTERNODE_MIN_MNB_SECONDS)) {
        ...
        if (fAcceptable) {
            ...
            // should be at least not earlier than block when 1000 BZE tx got MASTERNODE_MIN_CONFIRMATIONS
            uint256 hashBlock = uint256();
            CTransaction tx2;
            GetTransaction(vin.prevout.hash, tx2, Params().GetConsensus(), hashBlock, true);
            BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
            if (mi != mapBlockIndex.end() && (*mi).second) {
                CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 1000 BZE tx -> 1 confirmation
