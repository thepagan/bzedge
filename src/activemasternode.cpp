...
void CActiveMasternode::ManageStatus()
{
    ...
    if (ChainNameFromCommandLine() != CBaseChainParams::REGTEST && !masternodeSync.IsBlockchainSynced()) {
...
bool CActiveMasternode::SendMasternodePing(std::string& errorMessage)
{
    ...
    if (pmn != NULL) {
        ...
        int64_t masterNodeSignatureTime = GetTime();
...
bool CActiveMasternode::Register(std::string strService, std::string strKeyMasternode, std::string strTxHash, std::string strOutputIndex, std::string& errorMessage)
{
    ...
    {
        LogPrintf("Invalid address %s for masternode.", strService);
        return false;
    }
