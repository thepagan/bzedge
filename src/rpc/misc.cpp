...
extern CAmount getBalanceZaddr(std::string address, int minDepth = 1, int maxDepth = INT_MAX, bool ignoreUnspendable=true);
extern void ListTransactions(const CWalletTx& wtx, const string& strAccount, int nMinDepth, bool fLong, UniValue& ret, const isminefilter& filter);
extern void AcentryToJSON(const CAccountingEntry& acentry, const string& strAccount, UniValue& ret);
...
UniValue getalldata(const UniValue& params, bool fHelp)
{
    ...
    if (params.size() > 0 && (params[0].get_int() == 1 || params[0].get_int() == 0))
    {
      BOOST_FOREACH(const PAIRTYPE(CTxDestination, CAddressBookData)& item, pwalletMain->mapAddressBook)
      {
          ...
          addrlist.pushKV(keyIO.EncodeDestination(dest), addr);
      }
...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total " + COIN_NAME + " balance of the wallet\n"
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nReturn information about the given " + COIN_NAME + " address.\n"
            "\nArguments:\n"
            "1. \"address\"     (string, required) The " + COIN_NAME + " address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"address\",        (string) The " + COIN_NAME + " address validated\n"
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are " + COIN_NAME + " addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) " + COIN_NAME + " address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The " + COIN_NAME + " address to use for the signature.\n"
...
UniValue getaddressmempool(const UniValue& params, bool fHelp)
{
    ...
    if (!(fExperimentalInsightExplorer || fExperimentalLightWalletd)) {
        throw JSONRPCError(RPC_MISC_ERROR, "Error: getaddressmempool is disabled. "
            "Run './" + COIN_CLI_EXECUTABLE + " help getaddressmempool' for instructions on how to enable this feature.");
    }
...
UniValue getaddressutxos(const UniValue& params, bool fHelp)
{
    ...
    if (!(fExperimentalInsightExplorer || fExperimentalLightWalletd)) {
        throw JSONRPCError(RPC_MISC_ERROR, "Error: getaddressutxos is disabled. "
            "Run './" + COIN_CLI_EXECUTABLE + " help getaddressutxos' for instructions on how to enable this feature.");
    }
...
UniValue getaddressdeltas(const UniValue& params, bool fHelp)
{
    ...
    if (!(fExperimentalInsightExplorer || fExperimentalLightWalletd)) {
        throw JSONRPCError(RPC_MISC_ERROR, "Error: getaddressdeltas is disabled. "
            "Run './" + COIN_CLI_EXECUTABLE + " help getaddressdeltas' for instructions on how to enable this feature.");
    }
...
UniValue getaddressbalance(const UniValue& params, bool fHelp)
{
    ...
    if (!(fExperimentalInsightExplorer || fExperimentalLightWalletd)) {
        throw JSONRPCError(RPC_MISC_ERROR, "Error: getaddressbalance is disabled. "
            "Run './" + COIN_CLI_EXECUTABLE + " help getaddressbalance' for instructions on how to enable this feature.");
    }
...
UniValue getaddresstxids(const UniValue& params, bool fHelp)
{
    ...
    if (!(fExperimentalInsightExplorer || fExperimentalLightWalletd)) {
        throw JSONRPCError(RPC_MISC_ERROR, "Error: getaddresstxids is disabled. "
            "Run './" + COIN_CLI_EXECUTABLE + " help getaddresstxids' for instructions on how to enable this feature.");
    }
...
UniValue getspentinfo(const UniValue& params, bool fHelp)
{
    ...
    if (!fExperimentalInsightExplorer) {
        throw JSONRPCError(RPC_MISC_ERROR, "Error: getspentinfo is disabled. "
            "Run './" + COIN_CLI_EXECUTABLE + " help getspentinfo' for instructions on how to enable this feature.");
    }
