...
void UpdatePreferredDownload(CNode* node, CNodeState* state)
{
    ...
    state->fPreferredDownload = (GetBoolArg("-graylist", false) || !node->fInbound || node->fWhitelisted) && !node->fOneShot && !node->fClient;
    nPreferredDownload += state->fPreferredDownload;
}
...
bool GetSpentIndex(CSpentIndexKey &key, CSpentIndexValue &value)
{
    ...
    if (!pblocktree->ReadSpentIndex(key, value))
    {
        ...
        // we shouldn't spam the log on each unspent output
        return false;        
    }
...
void ThreadScriptCheck() {
    RenameThread(strprintf("%s-scriptch", COIN_NICKNAME).c_str());
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (!CheckBlock(block, state, chainparams, verifier, fCheckPoW && !fJustCheck, !fJustCheck, fCheckTransactions))
        return false;
...
void static ProcessGetData(CNode* pfrom, const Consensus::Params& consensusParams)
{
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        {
            LogPrintf("pfrom->nSendSize = %i > SendBufferSize() = %i\n", pfrom->nSendSize, SendBufferSize());
            break;
        }
...
bool static ProcessMessage(const CChainParams& chainparams, CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version")
    {
        ...
        if (pfrom->nVersion < MIN_PEER_PROTO_VERSION)
        {
			...
			CNode::Ban(pfrom->addr, BanReasonManuallyAdded);
            return false;
        }
    ...
    else if (strCommand == "block" && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        {
            LogPrintf("WARNING: Empty block message received from peer id=%i addr=%s\n", pfrom->id, pfrom->addr.ToString());
            // empty block message causes (handled) exception, should we ignore or raise ban score of the misbehaving peer ?        
        }
    }
